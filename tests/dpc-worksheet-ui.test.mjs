...
import {newState as priorWorksheet} from '../src/lib/dpc-worksheet-v3.mjs';
...
test('single open worksheet: blank/zero, custom rows, saving, exports, imports and failure handling',async()=>{
 const x=setup({'dpc:worksheet:v3:draft':JSON.stringify(priorWorksheet())});try{
  ...
  click('save-copy');assert.equal(JSON.parse(w.localStorage.getItem('dpc:worksheet:v4:saves')).length,1);edit('[data-input="members"]','450');click('load-copy');assert.equal(q('[data-input="members"]').value,'360');assert.equal(q('[data-row-field="label"]').value,'Weekend support');assert.equal(q('[data-meta="feedback"]').querySelector('img'),null);
...
test('older drafts and named copies preserve both columns and custom labels without changing legacy keys',async()=>{
 ...
 const x=setup({'dpc:model:v2:draft':raw,'dpc:model:v2:saves':named});try{const {q,edit,click,w}=x;assert.equal(q('[data-input="members"]').value,'210');assert.equal(q('[data-migration]').hidden,false);assert.equal(q('[data-earlier-select]').options.length,2);assert.equal(q('[data-saved-select]').options.length,2);edit('[data-input="members"]','220');q('[data-earlier-select]').value='0';click('open-earlier');assert.equal(q('[data-input="members"]').value,'450');assert.equal(q('[data-sheet-meta="name"]').value,'My future plan');q('[data-earlier-select]').value='0';click('open-earlier');assert.equal(q('[data-input="members"]').value,'220');click('restore-defaults');assert.equal(q('[data-sheet-meta="name"]').value,'My numbers');assert.equal(q('[data-meta="feedback"]').value,old.feedback);assert.equal(w.localStorage.getItem('dpc:model:v2:draft'),raw);assert.equal(w.localStorage.getItem('dpc:model:v2:saves'),named);const saved=w.localStorage.getItem('dpc:worksheet:v4:draft');const y=setup({'dpc:worksheet:v4:draft':saved});try{assert.equal(y.q('[data-earlier-select]').options.length,3);assert.equal(y.q('[data-meta="feedback"]').value,old.feedback);}finally{y.dom.window.close();}}finally{x.dom.window.close();}
});
...
test('staffing controls, stages, incomplete charts, saving and earlier-model switching',async()=>{
 const x=setup();try{
  const {q,w,edit,click}=x;assert.match(q('[data-engine-note]').textContent,/revised staffing/);assert.equal(w.document.querySelectorAll('.role-editor').length,3);assert.equal(w.document.querySelectorAll('[data-replaced]:not([hidden])').length,0);assert.equal(q('[data-result="safeMembers"]').textContent,'325');assert.equal(q('[data-result="surplus"]').textContent,'Not provided');assert.equal(q('[data-roster-body]').children.length,7);assert.equal(q('[data-monthly-body]').children.length,36);
  for(const k of ['extendedSupport','leaveRelief','recruiting','extendedOccupancy'])edit('[data-structured="settings.'+k+'"]','0');assert.equal(q('[data-result="surplus"]').textContent,'-$37,500');
  edit('[data-structured="roles.1.availableEnd"]','');assert.equal(q('[data-result="safeMembers"]').textContent,'Not provided');click('export-json');assert.equal(JSON.parse(await x.downloads[0].text()).worksheet.staffing.roles[1].availableEnd,null);edit('[data-structured="roles.1.availableEnd"]','21');
  q('[data-role-action="add-stage"][data-role-index="1"]').click();assert.equal(w.document.querySelectorAll('.stage-editor').length,1);edit('[data-structured="roles.1.stages.0.managementHours"]','2');edit('[data-structured="settings.matureMonth"]','24');assert.ok(Number(q('[data-result="safeMembers"]').textContent)<325);assert.equal(q('[data-result="physicianPay"]').textContent,'$700,000');
  edit('[data-structured="roles.1.stages.0.month"]','');click('export-json');assert.equal(JSON.parse(await x.downloads[1].text()).worksheet.staffing.roles[1].stages[0].month,null);edit('[data-structured="roles.1.stages.0.month"]','24');
  click('save-copy');await wait();const saved=w.localStorage.getItem('dpc:worksheet:v4:draft');const y=setup({'dpc:worksheet:v4:draft':saved});try{assert.equal(y.q('[data-structured="roles.1.stages.0.managementHours"]').value,'2');assert.equal(y.q('[data-structured="settings.matureMonth"]').value,'24');}finally{y.dom.window.close();}
  click('export-csv');assert.ok((await x.downloads[2].text()).includes('Management and training'));assert.ok(!q('[data-chart="cash"]').innerHTML.includes('NaN'));
  click('add-role');assert.equal(w.document.querySelectorAll('.role-editor').length,4);assert.equal(q('[data-result="costs"]').textContent,'Not provided');
  click('start-revised');assert.equal(w.document.querySelectorAll('.role-editor').length,3);assert.equal(q('[data-earlier-select]').options.length,2);q('[data-earlier-select]').value='0';click('open-earlier');assert.equal(w.document.querySelectorAll('.role-editor').length,4);
 }finally{x.dom.window.close();}
});
