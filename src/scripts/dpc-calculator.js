import {fields,groups,reference,metrics,launchMetrics,monthlyColumns,formatMetric,treatments,newState,blankRow,validateState,openPreserved,calculateWorksheet,calculateLaunchWorksheet,exportPack,reviewText,worksheetCsv,roleFields,settingFields,days,newRole,newStage,replacedKeys,startRevised,incentiveFields} from '../lib/dpc-worksheet.mjs';
const DRAFT='dpc:worksheet:v5:draft',SAVES='dpc:worksheet:v5:saves';
const oldDrafts=['dpc:worksheet:v4:draft','dpc:worksheet:v3:draft','dpc:model:v2:draft','metsi:model:v1:draft'],oldSaves=['dpc:worksheet:v4:saves','dpc:worksheet:v3:saves','dpc:model:v2:saves','metsi:model:v1:saves'];
export function initializeDpcWorksheet(){
 ...
 function renderStaffing(){
  ...
  if(state.worksheet.staffing.incentives){const incentives=document.createElement('section');incentives.className='staffing-settings';const h=document.createElement('h4');h.textContent='Associate profit sharing';incentives.append(h);for(const f of incentiveFields)structuredField(incentives,'incentives.'+f.key,f);const p=document.createElement('p');p.className='small';p.textContent='The employer-cost pool includes bonus payroll burden. Split gross bonuses equally among active associates and replacements; owner and relief roles do not participate. Modeled monthly payment, no commission on labs, prescriptions or products.';incentives.append(p);host.append(incentives);}else{const p=document.createElement('p');p.className='notice';p.textContent='Preserved prior worksheet: original pricing and salaries remain active; profit sharing is disabled. Use Start revised staffing proposal to open the new trial defaults.';host.append(p);}
 ...
 root.addEventListener('click',async event=>{
  ...
  if(action==='restore-defaults'){if(!window.confirm('Restore prefilled assumptions? The current worksheet is retained first; your names, added rows, notes and feedback stay.'))return;state.preservedWorksheets.push(structuredClone(state.worksheet));const fresh=newState().worksheet;state.worksheet.values={...fresh.values};state.worksheet.engine=fresh.engine;state.worksheet.staffing=fresh.staffing;state.worksheet.basis='Reference assumptions';state.worksheet.launchBasis='Reference assumptions';populate();save();announce('Prefilled figures restored. Added rows and notes are unchanged.');return;}
  if(!check())return;
