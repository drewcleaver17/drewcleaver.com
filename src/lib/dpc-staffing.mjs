// R05 roster model. Monetary totals deliberately propagate unknown inputs.
...
export const roleFields=[
 ...
 f('coreMultiplier','Core appointment-block multiplier',1,'times the shared Core block',0,10,'any','Adjust this role independently. 1 uses the shared block; 2 doubles it. The derived minutes appear in the roster summary.'),
 f('extendedMultiplier','Extended appointment-block multiplier',1,'times the shared Extended block',0,10,'any','Role-specific blocks change capacity. Pooled workload allocates appointments in proportion to role capacity.'),
...
export function validateStaffing(raw){
 ...
 return {settings:numeric(raw.settings,settingFields),weights:[...raw.weights],launchAccess:raw.launchAccess,enrollmentMode:raw.enrollmentMode,roles};
}
...
export function rosterCapacity(worksheet,month,{partial=false}={}){
 ...
 const block=known(v.extendedMix)&&known(v.coreBlock)&&known(v.extendedBlock)?v.coreBlock*(1-v.extendedMix/100)+v.extendedBlock*v.extendedMix/100:null;
 ...
 for(const r of active){
  ...
  if(good){
   ...
   roleBlock=known(v.extendedMix)&&known(v.coreBlock)&&known(v.extendedBlock)?v.coreBlock*r.coreMultiplier*(1-v.extendedMix/100)+v.extendedBlock*r.extendedMultiplier*v.extendedMix/100:null;
   if(!known(roleBlock)||roleBlock<=0||(v.extendedMix<100&&v.coreBlock*r.coreMultiplier===0)||(v.extendedMix>0&&v.extendedBlock*r.extendedMultiplier===0)){complete=false;warnings.push(r.name+': an active appointment type needs a positive block.');}
 ...
 if(active.length>3)warnings.push('This month has '+active.length+' paid physicians. Replacement/relief overlap exceeds the proposed three-physician steady team.');
 ...
 daily.forEach((d,i)=>{d.weight=known(total)&&total>0?included[i]/total:null;d.routineCapacity=complete?d.routineMinutes/block:null;});
...
export function evaluateDemand(w,roster,members,newMembers=0,{target=true}={}){
 ...
 const daily=roster.daily.map(d=>{
  ...
  const p=Math.max(ratio(minutes*d.weight,d.routineMinutes*factor),ratio(encounters*d.weight,d.routineCount*factor),ratio((encounters+members*z.urgentRate/100)*d.weight,d.totalCount*factor),ratio(urgent*d.weight+esc,d.urgentMinutes*factor));pressure=Math.max(pressure,p);
  return {...d,routineDemand:routine*d.weight,onboarding:newMembers*d.weight,urgentDemand:members*z.urgentRate/100*d.weight,pressure:Number.isFinite(p)?p*100:null,over:p>1+1e-7};
 });
 pressure=Math.max(pressure,ratio(escalation,roster.ownerUrgent*factor));
 const activeHours=sum(...roster.daily.map(d=>d.routineMinutes>0?minutes*d.weight*d.bookableHours/d.routineMinutes:0))+urgent/60+escalation/60+sum(...roster.daily.map(d=>d.adminHours+d.managementHours+d.otherHours));
