export const MODEL_VERSION = 'metsi-2026-09-v1';
...
export const groups = [
 ...
 f('members','Active members',360,'members',0,1000000,1),
 ...
 f('startupCapital','One-time startup capital',null,'$',0,100000000,'any','Optional. Blank means not supplied, not zero.',true),f('workingCapital','Opening working-capital reserve',null,'$',0,100000000,'any','Optional. Kept separate from recurring operating costs.',true)]},
...
export function formatMetric(v,type){if(v==null)return type.startsWith('money')?'Not provided':'—';return type==='money2'?new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',minimumFractionDigits:2,maximumFractionDigits:2}).format(v):type==='money'?money(v):type==='percent'?number(v)+'%':type==='count'?Math.ceil(v).toLocaleString('en-US'):number(v);}
...
export function newState(){return {schemaVersion:1,modelVersion:MODEL_VERSION,name:'METSI Care discussion',author:'',updatedAt:null,scenarios:[{name:'Garrick’s inputs',basis:'Reference assumptions',period:'',notes:'',values:{...reference}},{name:'Proposed scenario',basis:'Reference assumptions',period:'',notes:'',values:{...reference}}],feedback:''};}
const bases=['Reference assumptions','Ballpark estimates','Actuals and estimates','Actual practice figures'];
export function validateState(raw){if(!raw||typeof raw!=='object'||raw.schemaVersion!==1||raw.modelVersion!==MODEL_VERSION)throw new Error('This file uses an unsupported model version. Export a new review file from this tool.');if(!Array.isArray(raw.scenarios)||raw.scenarios.length!==2)throw new Error('The file must contain both editable scenarios.');const state=newState();for(const key of ['name','author','feedback']){if(typeof raw[key]!=='string'||raw[key].length>20000)throw new Error('Invalid discussion text.');state[key]=raw[key];}for(let i=0;i<2;i++){const s=raw.scenarios[i];if(!s||!bases.includes(s.basis))throw new Error('Invalid input basis.');for(const key of ['name','period','notes'])if(typeof s[key]!=='string'||s[key].length>20000)throw new Error('Invalid scenario text.');const errors=validateValues(s.values);if(errors.length)throw new Error(errors[0]);state.scenarios[i]={name:s.name,basis:s.basis,period:s.period,notes:s.notes,values:Object.fromEntries(fields.map(f=>[f.key,s.values[f.key]]))};}state.updatedAt=typeof raw.updatedAt==='string'?raw.updatedAt:null;return state;}
export function exportPack(state){const s=validateState(state);return {...s,exportedAt:new Date().toISOString(),reference:{...reference},results:{reference:calculate(reference),scenarios:s.scenarios.map(s=>calculate(s.values))},modelNotes:['Annual compensation is entered, not prorated by days.','Capacity uses the smaller of appointment cap and available hours, weighted by membership mix.','Platform fees are an expense to practices and revenue to the platform; combined surplus eliminates the transfer.','Pilot dues are prorated by 365 days and exclude pilot costs. This is not pilot profit.','Local saving stays in this browser. Export and import are the exchange mechanism.']};}
export function reviewText(state){const pack=exportPack(state);const lines=['METSI CARE — SCENARIO REVIEW',pack.name,`Prepared by: ${pack.author||'Not supplied'}`,`Exported: ${pack.exportedAt}`,`Model: ${MODEL_VERSION}`,''];for(let i=0;i<2;i++){const s=pack.scenarios[i],r=pack.results.scenarios[i];lines.push(s.name,`Basis: ${s.basis}; period: ${s.period||'Not supplied'}`,...metrics.map(([k,label,type])=>`${label}: ${formatMetric(r[k],type)}`),'',...fields.map(f=>`${f.label}: ${s.values[f.key]??'Not supplied'} ${f.unit} (reference: ${f.value??'not supplied'})`),'',`Notes: ${s.notes||'None'}`,'Checks:',...r.warnings.map(w=>'- '+w),'');}lines.push('FEEDBACK / QUESTIONS',pack.feedback||'None');return lines.join('\n');}
const csvCell=x=>{let s=String(x??'');if(typeof x==='string'&&/^[\s]*[=+@-]/.test(s))s="'"+s;return '"'+s.replaceAll('"','""')+'"';};
export function comparisonCsv(state){const p=exportPack(state),rs=[p.results.reference,...p.results.scenarios];const rows=[['METSI Care comparison',p.name],['Model version',MODEL_VERSION],['Author',p.author],['Basis','Reference assumptions',...p.scenarios.map(s=>s.basis)],['Period','Illustrative mature practice',...p.scenarios.map(s=>s.period)],['Assumption','Units','Reference',...p.scenarios.map(s=>s.name)]];for(const f of fields)rows.push([f.label,f.unit,f.value??'Not supplied',...p.scenarios.map(s=>s.values[f.key]??'Not supplied')]);rows.push([],['Calculated result','Units','Reference',...p.scenarios.map(s=>s.name)]);for(const [k,label,type] of metrics)rows.push([label,type,...rs.map(r=>r[k]??'Not provided')]);rows.push([],['Scenario notes','',...p.scenarios.map(s=>s.notes)],['Feedback',p.feedback]);return '\uFEFF'+rows.map(r=>r.map(csvCell).join(',')).join('\r\n');}
